...
## Modular V1.5 — safe-shift and pocket validation

Three Modular V1.4 runs all completed at roughly 520.2s, 454.5s, and 441.4s, with death counts 8, 5, and 2. V1.4 therefore remains the routing/DPS baseline.

Remaining evidence-driven failures:

- Sanada could still execute a generic 10-stud boss-wave shift from `old_inside=3` to `new_inside=2`, then die before the specialized crossShuriken pocket could recover.
- committed Sanada pockets could be created and immediately released several times in a few tenths of a second because fresh candidate selection merely scored heavy physical pressure instead of rejecting it;
- Miyamoto `flameBeams` warnings often arrived while an older shuriken/cyclone boss wave was still active, preventing the V1.4 pre-move from running.

V1.5 changes:

- Samurai Palace boss direct shifts may not commit if the bounded endpoint still overlaps active attack geometry;
- fresh Samurai boss pocket candidates hard-reject physical-pressure values at or above the existing hard threshold before they can be committed;
- Miyamoto beam-warning pre-move may run during an older boss wave when no real flameBeam/doubleFlameBeam geometry is currently active and the short route is safe;
- no change to V1.3/V1.4 navigation, floor-transition ownership, 3-second global shift governor, no-tween rule, or authoritative completion.
