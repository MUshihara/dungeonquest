--[[
    ...
    SAMURAI PALACE INTELLIGENT COMBAT V1 — EVIDENCE-FIRST ADAPTER
    ------------------------------------------------------------
    Development target:
      Samurai Palace
      Shared gameplay PlaceId observed: 85776757589518
      UniverseId observed: 9931749389

    Evidence source:
      Full passive Samurai Palace recon captured 2026-09-19.

    Proven Samurai Palace topology:
      • workspace.dungeon.room1 ... room9 + bossRoom
      • room 4 boss: Sanada Yukimura
      • room 7 boss: Ancient Golem Guardian
      • bossRoom: Miyamoto Musashi
      • normal enemies: Samurai Swordsman / Shuriken Thrower
      • boss adds: Elite Swordsman / Ultimate Swordsman

    Proven dynamic hazards:
      • npcShurikenThrow
      • shurikenThrow
      • crossShuriken
      • eliteSwordsmanSpin
    ...
    V1 goals:
      1. Preserve the proven Underworld V10.x shared engine.
      2. One physical movement owner.
      3. Attack while dodging when safe.
      4. Strongest/local-finish targeting with hard boss priority.
      5. Treat Shuriken Thrower as a geometry/ranged-pressure enemy.
      6. Treat Samurai/Elite/Ultimate swordsmen as physical pressure.
      7. Predict Ancient Golem Guardian rockshatter from its observed Action tell.
      8. Observe Miyamoto client events passively; never invent/fireserver them.
      9. Walk/strafe first; bounded direct shift only when necessary.
      10. Keep the global direct-shift governor at 3.00 seconds.
      11. No tween transit.

    IMPORTANT:
      • TEST BUILD — first Samurai Palace combat adapter.
      • No anti-cheat bypasses.
      • No anti-kick hooks.
      • No lobby automation.
      • No purchases.
      • No admin/dev remotes.
      • No hidden remote signatures.
      • No network-ownership bypass.
      • No vertical/floating dodge.

    Runtime logs:
      DQR_Samurai_Palace_Test_V1/
        Samurai_Palace_Combat_V1_<timestamp>.txt
]]
...
-- This adapter is intentionally world-specific. The shared DQR battle place
-- hosts multiple dungeons, so PlaceId alone is not enough; dungeonName is the
-- authoritative world identity observed in the Samurai Palace recon.
do
    local dungeonNameValue = workspace:FindFirstChild("dungeonName")

    if dungeonNameValue
        and tostring(dungeonNameValue.Value) ~= ""
        and tostring(dungeonNameValue.Value) ~= "Samurai Palace"
    then
        warn(
            "[DQR Samurai V1] Wrong dungeon: "
            .. tostring(dungeonNameValue.Value)
            .. " (expected Samurai Palace). Controller not started."
        )
        return
    end
end
...
local RUNTIME_KEY = "__SERENITY_DQR_SAMURAI_PALACE_COMBAT_V1"
...
for _, oldKey in ipairs({
    ...
    if previous and type(previous.Stop) == "function" then
        pcall(previous.Stop, "samurai_palace_v1_takeover")
    end
end
...
Runtime.IsRangedPressureName = function(name)
    return tostring(name or "") == "Shuriken Thrower"
end
...
Runtime.IsSamuraiBossName = function(name)
    ...
    return
        ...
        or name == "Ancient Golem Guardian"
        or name == "Miyamoto Musashi"
end
...
local BASE = "DQR_Samurai_Palace_Test_V1"
local LOG_PATH =
    BASE .. "/Samurai_Palace_Combat_V1_" .. tostring(os.time()) .. ".txt"
...
local function refreshBossState()
    ...
    if newName ~= Runtime.ActiveBossName then
        ...
        if newName == "Demon Lord Azrallik"
            or newName == "Miyamoto Musashi"
        then
            Runtime.SawFinalBoss = true
        end

        if (
                oldName == "Demon Lord Azrallik"
                or oldName == "Miyamoto Musashi"
            )
            and newName == nil
...
ENV.DQRSamuraiPalace = {
    Version = "V1-Evidence-First",

    Status = function()
        return {
            Style = CFG.STYLE,
            Dungeon = "Samurai Palace",
            Room = Runtime.CurrentRoomIndex,
...
log(
    "SAMURAI_KNOWN_ATTACKS",
    "npcShurikenThrow,shurikenThrow,crossShuriken,eliteSwordsmanSpin,"
    .. "golemRockThrow,golemRockThrowSmall,golemRockClap,rockshatter,"
    .. "rockExplosion,rockExplosionSmall,flameShurikenHit,flameBeam,"
    .. "doubleFlameBeam,Flame Cyclone"
)
